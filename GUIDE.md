...
> Receive Okta telephony hooks and submit SMS messages to Azure Communication Services.

## Status

Integration sample. Unit tests cover authentication and ACS send results; a live
Okta/ACS deployment and latency validation are required before production use.
OAuth and a delivery-report consumer are not implemented.

## Verification

After installing requirements, run `python -m unittest discover -s tests -v`.
The Quality workflow runs these tests without credentials or sending SMS.

## Release and Rollback

Configure secrets before publishing to the intended Function App. Missing
`OKTA_BASIC_SECRET` now rejects requests with HTTP 401. Keep the previous
deployment artifact and use a staging slot where supported. Verify authentication,
hook response timing, and carrier delivery before switching production traffic.
...
- **Response deadline** - Measure cold and warm requests against Okta's timeout
- **Authentication** - Required Basic authentication; missing secrets fail closed
- **Delivery Tracking** - Optional Event Grid integration for delivery reports
- **Error handling** - Authorized requests return a hook outcome; unauthorized requests return HTTP 401
...
- Basic authentication configured with the username `okta`
...
**Development Tools:**
- Python 3.12 (local test baseline; use a supported Azure Functions runtime)
...
|---------------------------|-----------------------------------------------------------|--------------------------------------------------|
| Username                 | Fixed in code, not an environment setting                | `okta`                                           |
...
**Status Values:**
- `SUCCESS` - ACS accepted the SMS; this is not proof of carrier delivery
- `FAILED` - Authorized request could not be sent (missing config, ACS error, etc.)

Missing or invalid Basic credentials receive HTTP 401.
...
Unhandled exceptions are logged through Python logging. Configure Application
Insights on the Function App to collect these logs. Do not enable SDK payload
logging: phone numbers, OTPs, and authorization headers must remain private.
...
**Sample integration: validate against your security and availability requirements.**
